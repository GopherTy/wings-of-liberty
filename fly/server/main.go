...
func grpcArrayService() (arr *encryption.EncrypArray, err error) {
	conn, err := grpc.Dial(":10000", grpc.WithInsecure())
	if err != nil {
		log.Fatalln("grpc service start fail ", err)
	}
	gClient := array.NewArrayClient(conn)
	resp, err := gClient.GetEncryptionArray(context.Background(), &array.ArrayRequest{})
	if err != nil {
		log.Fatalln("recive grpc service data fail ", err)
	}
	if resp.Array == nil {
		log.Fatalln("array is nil")
	}
